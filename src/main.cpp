...
void OBJ_loader_BVH_test(int width, int height){
	...
	Transform identity;
	std::vector<std::shared_ptr<Shape>>	floor_mesh = CreateTriangleVector(&identity, &identity, true, 
	...
	int nTrisLight = 2;
	int nVersLight = 4;
	const float yPosLight = 10.f; 
	Point3f pLight[] = {Point3f(-12.f, yPosLight, -8.f), Point3f(-12.f, yPosLight, 2.f), 
				Point3f(-2.f, yPosLight, 2.f), Point3f(-2.f, yPosLight, -8.f)};
	...
	std::shared_ptr<Sampler> halsamp = std::make_shared<HaltonSampler>(60, imageBound, false);
